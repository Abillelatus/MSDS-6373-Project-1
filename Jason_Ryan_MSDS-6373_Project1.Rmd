...
lasso_x <- life_exp_train_v05$Life.expectancy

# Create a data subset because slicing an existing data frame to make a matrix is 
# a huge pain in the keyboard
life_exp_train_v05.1 <- subset(life_exp_train_v05, select=c(-Life.expectancy))
lasso_y <- data.matrix(life_exp_train_v05.1)

# Solve for missing data using mice 
lasso_y_mice <- mice(lasso_y, solve())
...
---- Start Solo Analysis ----
If the data was not split from above then go ahead and split it here
```{r}
if (split_data == FALSE) {
  # Sperate the data 
  set.seed(1536)
  split_size <- floor(0.85 * nrow(life_exp_train_v05))
  train_ind <- sample(seq_len(nrow(life_exp_train_v05)), size = split_size)
  life_exp_train_final <- life_exp_train_v05[train_ind,]
  life_exp_test <- life_exp_train_v05[-train_ind,]
}
```
